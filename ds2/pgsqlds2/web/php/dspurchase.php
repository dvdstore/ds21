...
if (empty($confirmpurchase))
  ...
  for ($i=0; $i<count($item); $i++)
    ...
    if (empty($drop) || !in_array($i, $drop))
      {
      ++$j;
      $purchase_query = "select PROD_ID, TITLE, ACTOR, PRICE from PRODUCTS where PROD_ID=$item[$i]";
/*      pgi_real_query($link_id, $purchase_query);*/
      $purchase_result = pg_query($purchase_query);
  ...
  $command_result = pg_query("START TRANSACTION");
  $purchase_insertorder_query = "INSERT into ORDERS (ORDERDATE, CUSTOMERID, NETAMOUNT, TAX, TOTALAMOUNT)" .
    " VALUES ( '$currentdate',$customerid,$netamount_fmt,$taxamount_fmt,$totalamount_fmt) RETURNING orderid;";
  $purchase_insertorder_result = pg_query($purchase_insertorder_query);
  $result_row = pg_fetch_row($purchase_insertorder_result);
  // $orderid = pg_insert_id($link_id);
  // $seqquery = "SELECT CURRVAL('orders_orderid_seq');";
  //$result = pg_query($seqquery);
  //$result_row = pg_fetch_row($result);
  pg_free_result($purchase_insertorder_result);
...
*/
  // loop through items to be purchased, check inventory,  and make inserts into ORDERLINES and CUST_HIST tables     
    ...
    $success = true;
    $orderlines_insert = "INSERT into ORDERLINES (ORDERLINEID, ORDERID, PROD_ID, QUANTITY, ORDERDATE) VALUES "; 
    $cust_hist_insert= "INSERT into CUST_HIST (CUSTOMERID, ORDERID, PROD_ID) VALUES ";
    if ($success)
    {
    for ($i=0; $i < count($item); $i++)
      {
      $j = $i+1;
      $query = "SELECT QUAN_IN_STOCK, SALES FROM INVENTORY WHERE PROD_ID=$item[$i]";
      ...
      if ($new_quan < 0)
        ...
        {
        $query = "UPDATE INVENTORY SET QUAN_IN_STOCK=$new_quan, SALES=$new_sales WHERE PROD_ID=$item[$i];";
        $result = pg_query($query);
        }
    ...
    if ($success)
      ...
      $cc_query = "select CREDITCARDTYPE, CREDITCARD, CREDITCARDEXPIRATION from CUSTOMERS where CUSTOMERID=$customerid";
      $cc_result = pg_query($cc_query);

...
if (!(empty($username)))
  ...
  if (pg_num_rows($result) > 0)
    ...
    $query = "SELECT PRODUCTS.TITLE, PRODUCTS.ACTOR, PRODUCTS.COMMON_PROD_ID" .
             " FROM CUST_HIST INNER JOIN PRODUCTS ON CUST_HIST.PROD_ID = PRODUCTS.PROD_ID" .
             " WHERE CUST_HIST.CUSTOMERID =" . $customerid . " ORDER BY ORDERID DESC, TITLE ASC LIMIT 10;";
/*    pgi_real_query($link_id, $query); */
    $result = pg_query($query);
    ...
    if (pg_num_rows($result) > 0)
      ...
      while ($result_row = pg_fetch_row($result))
        {
        $query2 = "select TITLE from PRODUCTS where PROD_ID =" . $result_row[2] . ";";
/*        pgi_real_query($link_id, $query2);*/
/*        $result2 = pgi_store_result($link_id);*/

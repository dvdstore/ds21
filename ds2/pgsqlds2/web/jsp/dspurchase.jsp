<!-- 
dspurchase.jsp jsp page that purchases an order from the DVD store by entering the order into a database running on pgsql
...
if (confirmpurchase == null)              // if not confirmiming purchase - print list to allow for
  ...
  try
    ...
    orderconn = DriverManager.getConnection(jdbcURL); 
    ...
    purchase_insert_query = "INSERT into ORDERS (ORDERDATE, CUSTOMERID, NETAMOUNT, TAX, TOTALAMOUNT)" +
  ...
  // loop through purchased items and make inserts into orderdetails table     
    ...
    String query;
    success = true;
    String p_query = "INSERT into ORDERLINES (ORDERLINEID, ORDERID, PROD_ID, QUANTITY, ORDERDATE) VALUES"; 
    String c_insert= "INSERT INTO CUST_HIST (CUSTOMERID, ORDERID, PROD_ID) VALUES ";
    while (h < item.length)
      {
      j = h+1;
      query = "SELECT QUAN_IN_STOCK, SALES FROM INVENTORY WHERE PROD_ID=" + item[h] + ";";
      Connection quanconn = DriverManager.getConnection(jdbcURL); 
      ...
      if (new_quan < 0)                                     // if insufficient stock on hand - then flag failure
        ...
        {
        query = "UPDATE INVENTORY SET QUAN_IN_STOCK=" + new_quan + ", SALES=" + new_sales + " WHERE PROD_ID=" + item[h] + ";";
  ...
  if (success == true)
    {
      ...
      String cc_query = "select CREDITCARDTYPE, CREDITCARD, CREDITCARDEXPIRATION from CUSTOMERS where CUSTOMERID=" + customerid + ";";


      Connection queryconn = DriverManager.getConnection(jdbcURL); 

<!-- 
dslogin.jsp JSP page that validates login to DVD store on pgsql database
  ...
  if (username != null)          // if username and password have been entered, check if valid
    {
      ...
      if (userqueryResult.next())  // if user exists, then print out previous purchases and recommendataions
	  {
          ...
          <%
          query = "select PROD_ID FROM CUST_HIST where CUSTOMERID=" + customerid + " ORDER BY ORDERID DESC LIMIT 10;";         
          ...
          if (prevprod_queryResult.next())
            ...
            do
              ...
              prevproddetailResult = prevproddetail_queryStatement.executeQuery("select TITLE, ACTOR from PRODUCTS where PROD_ID=" + prev_id + ";");
              ...
              relatedprodResult = relatedprod_queryStatement.executeQuery("select TITLE from PRODUCTS where PROD_ID= (select COMMON_PROD_ID from PRODUCTS where PROD_ID=" + prev_id + ");");

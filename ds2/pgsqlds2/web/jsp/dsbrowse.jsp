<!-- 
dsbrowse.jsp jsp page that browses DVD store by author, title, or category on pgsql database
...
if (browsetype != null)          // if a browse type of either TITLE AUTHOR or CATEGORY was selected
  ...
  Connection conn = DriverManager.getConnection(jdbcURL);  // connect to pgsql 
  String browse_query = "";     // init string variable
  switch (browsetype.charAt(0))  // switch on browsetype that was selected
    {
    case 't':
      browse_query = "select * from PRODUCTS where  to_tsvector('simple',TITLE) @@ to_tsquery( '" + browse_title.replaceAll(" ","&") + "') LIMIT " + limit_num + ";";
      break;
    case 'a':
      browse_query = "select * from PRODUCTS where to_tsvector('simple',ACTOR) @@ to_tsquery('" + browse_actor.replaceAll(" ","&") + "') LIMIT " + limit_num + ";";
      break;
    case 'c':
      browse_query = "select * from PRODUCTS where CATEGORY = " + browse_category + " and SPECIAL=1 LIMIT " + limit_num + ";";
      break;
    }
   // query syntax used for TITLE and ACTOR is specific to doing a fulltext search on the PRODUCTS table
  Statement browseStatement = conn.createStatement();
  ResultSet browseResult = browseStatement.executeQuery(browse_query);    // submit browse query to pgsql                             

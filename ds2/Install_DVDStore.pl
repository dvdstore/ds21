...
{		
	if($bln_is_DB_ORACLE == 1)
	{
		...
		if((scalar(@arr1_db_file_paths) != 1 && scalar(@arr1_db_file_paths) < 4) || (scalar(@arr1_db_file_paths) > 4))
		{
			print "\nWrong number of paths entered!!!! \n";
			print "\nFour paths for following are needed:  Cust table dbfile, Index dbfile, DS_MISC dbfile, Order table dbfile respectively \n";
			exit(0);
		}
...
# On linux ds2 folder will be at root
# /
# /ds2/
# /ds2/data_files/
# /ds2/data_files/cust/
# /ds2/data_files/orders/
# /ds2/data_files/prod/
# /ds2/drivers/
# /ds2/mysqlds2/
# /ds2/mysqlds2/build/
...
#On Windows ds2 folder will be at <Driveletter>: and rest of folder structure will be same
...
{
	system("./ds2_create_cust $par1_Start $par2_End $par3_Fname $par4_DB_Size $par_Sys_Type");		
}
else   #Windows
{
	if($bln_IsCygwin == 1)  #Run through Cygwin Bash Shell
	{
		system("./ds2_create_cust.exe $par1_Start $par2_End $par3_Fname $par4_DB_Size $par_Sys_Type");	
	}
...
{	
	...
	if(lc($^O) eq lc("linux"))   #If system on which perl script is executing is Linux
	{
		system("./ds2_create_orders $par1_Start $par2_End $par3_ArrMonth[$i_LoopCount] $par4_DB_Size $par5_Month_Number $par_Sys_Type $par_Max_Prod_Id $par_Max_Cust_Id");		
	}
	else   #Windows
	{
		if($bln_IsCygwin == 1)  #Run through Cygwin Bash Shell
		{
			system("./ds2_create_orders.exe $par1_Start $par2_End $par3_ArrMonth[$i_LoopCount] $par4_DB_Size $par5_Month_Number $par_Sys_Type $par_Max_Prod_Id $par_Max_Cust_Id");	
		}
		else  #Run through DOS Shell
		{
			system("ds2_create_orders.exe $par1_Start $par2_End $par3_ArrMonth[$i_LoopCount] $par4_DB_Size $par5_Month_Number $par_Sys_Type $par_Max_Prod_Id $par_Max_Cust_Id");				
...
if(lc($^O) eq lc("linux"))   #If system on which perl script is executing is Linux
{
	system("./ds2_create_inv $par_n_Prod $par_Sys_Type");
...
else   #Windows
{
	if($bln_IsCygwin == 1)  #Run through Cygwin Bash Shell
	{
		system("./ds2_create_inv.exe $par_n_Prod $par_Sys_Type");
           system("cp inv.csv ../prod/");	
	}
	else  #Run through DOS Shell
	{
...
if(lc($^O) eq lc("linux"))   #If system on which perl script is executing is Linux
{
	system("./ds2_create_prod $par_n_Prod $par_Sys_Type");		
}
else   #Windows
{
	if($bln_IsCygwin == 1)  #Run through Cygwin Bash Shell
	{
		system("./ds2_create_prod.exe $par_n_Prod $par_Sys_Type");	
	}
	else   #Run through DOS Shell
	{
		system("ds2_create_prod.exe $par_n_Prod $par_Sys_Type");	
...
elsif($bln_is_DB_ORACLE == 1) 		#For Oracle
{
	...
	chdir "../../oracleds2/";		#Move to oracle directory
	...
	$line = "";
	$str_file_name = "";
	open (FILE, "oracleds2_cleanup_generic_fk_disabled_template.sql") || die "Can not Open file : $!";	
	@lines =  <FILE>;
	close (FILE);
	foreach $line (@lines)
	{
	...
	$line = "";
	$str_file_name = "";
	open (FILE, "oracleds2_cleanup_generic_template.sql") || die "Can not Open file : $!";	
	@lines =  <FILE>;
	close (FILE);
	foreach $line (@lines)
	{
	...
	$line = "";
	$str_file_name = "";
	open (FILE, "oracleds2_cleanup_generic_template.sh") || die "Can not Open file : $!";	
	@lines =  <FILE>;
	close (FILE);
	$str_file_name = "oracleds2_cleanup_".$database_size.$database_size_str.".sql";
	foreach $line (@lines)
	...
	$line = "";
	$str_file_name = "";
	open (FILE, "oracleds2_cleanup_generic_fk_disabled_template.sh") || die "Can not Open file : $!";	
	@lines =  <FILE>;
	close (FILE);
	$str_file_name = "oracleds2_cleanup_".$database_size.$database_size_str."_fk_disabled.sql";
	foreach $line (@lines)
	...
	$line = "";
	$str_file_name = "";
	open (FILE, "oracleds2_create_db_generic_template.sql") || die "Can not Open file : $!";	
	@lines =  <FILE>;
	close (FILE);
	foreach $line (@lines)
	{
	...
	$line = "";	
	$str_file_name = "";
	open (FILE, "oracleds2_create_all_generic_template.sh") || die "Can not Open file : $!";	
	@lines =  <FILE>;
	close (FILE);
	foreach $line (@lines)
	{
...
elsif($bln_is_DB_MSSQL == 1) 		#For SQL Server
{
	...
	chdir "../../sqlserverds2/";		#Move to oracle directory
	
	#Create new create_all sql script file from template
	@lines = ();
	$line = "";	
	$str_file_name = "";
	open (FILE, "sqlserverds2_create_all_generic_template.sql") || die "Can not Open file : $!";	
	@lines =  <FILE>;
	close (FILE);
	my $i_Cnt = 0;
	...
	$line = "";	
	$str_file_name = "";
	open (FILE, "sqlserverds2_cleanup_generic_template.sql") || die "Can not Open file : $!";	
	@lines =  <FILE>;
	close (FILE);
	foreach $line (@lines)
	{
	...
	$line = "";	
	$str_file_name = "";
	open (FILE, "sqlserverds2_create_ind_generic_template.sql") || die "Can not Open file : $!";	
	@lines =  <FILE>;
	close (FILE);
	my $i_Cnt = 0;
	my $i_arrCnt = scalar(@arr_db_file_paths);
	...
	$line = "";	
	$str_file_name = "";
	open (FILE, "sqlserverds2_create_db_generic_template.sql") || die "Can not Open file : $!";	
	@lines =  <FILE>;
	close (FILE);
	my $i_Cnt = 0;
	my $i_arrCnt = scalar(@arr_db_file_paths);

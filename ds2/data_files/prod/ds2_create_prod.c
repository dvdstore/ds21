...
int main(int argc, char* argv[])
  ...
  //if (argc < 2)
    ...
    fprintf(stderr, "Syntax: ds2_create_prod n_prods n_Sys_Type\n");  //Changed by GSK
...
//printf("title= %s  actor= %s\n", title, actor);
    ...
    {	
	fprintf(FP_prod, "%d,%d,%s,%s,%5.2f,%d,%d%c", prod_id, category, title, actor, price, special, common_prod_id, 10);
    }	
    else if(i_Sys_Type == 1) //If System is Windows, Append CR+LF   //Added by GSK   
    {
	fprintf(FP_prod, "%d,%d,%s,%s,%5.2f,%d,%d%c%c", prod_id, category, title, actor, price, special, common_prod_id, 13, 10);
    }
